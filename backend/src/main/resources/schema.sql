-- =====================================================================
-- Placement Management System - Complete Relational Database Schema
-- Compatible with PostgreSQL 14+ and H2 Database (Dev Mode)
-- Tables: 24 Tables across 6 Functional Domains
-- =====================================================================
...
-- 13. Job Eligible Departments Table
...
-- 16b. Job Skills Table
-- Normalises the skills a job asks for into rows (1NF) instead of a CSV column,
-- so they can be indexed, filtered and matched against student_skills.
CREATE TABLE IF NOT EXISTS job_skills (
    ...
    CONSTRAINT fk_job_skill_job FOREIGN KEY (job_id) REFERENCES jobs(id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
-- 23. Notification Recipients Table
...
-- 24. Notification Attachments Table
...
-- so constraint changes for already-provisioned databases go here.
-- Drop-then-add keeps each block re-runnable on every startup.
-- ---------------------------------------------------------------------

-- Widen notification_type to include APPLICATION (per-application alerts).
ALTER TABLE notifications DROP CONSTRAINT IF EXISTS chk_notifications_type;
ALTER TABLE notifications ADD CONSTRAINT chk_notifications_type
    CHECK (notification_type IN ('GENERAL', 'JOB', 'APPLICATION', 'PLACEMENT_DRIVE', 'INTERVIEW', 'DEADLINE', 'ANNOUNCEMENT', 'SYSTEM'));
